...
# img = lab.load_greyscale_image(os.path.join(TEST_DIRECTORY, 'test_images', 'pattern.png'))
# print(img)

def test_inverted_2():
    ''' Modify this function to implement the test described earlier (inverting 
    a small 1x4 image). Within this test case, define the expected result using 
    a predefined dictionary instance and compare it against the result obtained 
    from calling the inverted function on the original image. 
    You can utilize our compare_images function to facilitate the comparison
    '''
    img = lab.load_greyscale_image(os.path.join(TEST_DIRECTORY, 'test_images', 'pattern.png'))
    result = lab.inverted(img)
    excepted = {
        'height': 6,
        'width': 11,
        'pixels': [0, 113, 113, 115, 0, 0, 0, 115, 113, 113, 0,
                   0, 115, 113, 115, 0, 0, 0, 115, 113, 115, 0,
                   0, 115, 113, 115, 0, 0, 0, 115, 113, 115, 0,
                   255, 115, 113, 115, 255, 255, 255, 115, 113, 115, 255, 
                   255, 115, 113, 115, 255, 255, 255, 115, 113, 115, 255,
                   255, 115, 113, 113, 255, 255, 255, 113, 113, 115, 255 ],
    }

    compare_images(result, excepted)
    # assert False
